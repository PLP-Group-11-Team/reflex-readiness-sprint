...
export const ReflexProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Login handler with simulated credential check
  const login = useCallback(
    async (email: string, password?: string): Promise<{ success: boolean; error?: string }> => {
      ...
      if (matchedUser.role === 'rider' && matchedUser.riderId) {
        setActiveRiderId(matchedUser.riderId);
      }
      
      // Reset navigation tabs to role home
      setRetailerTab('dashboard');
      setDispatcherTab('dashboard');
      
      addToast(`Welcome back, ${matchedUser.name}! (${matchedUser.role.toUpperCase()})`, 'success');
      return { success: true };
    },
    [users, addToast]
  );
